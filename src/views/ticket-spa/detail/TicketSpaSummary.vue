...
import ModalTicketClinicReturnProduct from './modal/ModalTicketSpaReturnProduct.vue'
import { ticketSpaInvoiceHtmlContent } from './print-content/ticket-spa-invoice-html-print-content'
...
watch(
  () => ticketRef.value.ticketProductList,
...
const disabledSave = computed(() => {
  if ([TicketStatus.Debt, TicketStatus.Completed].includes(ticketRef.value.ticketStatus)) {
    return true
  }
  if (
    TicketProduct.equalList(ticketProductList.value, ticketRef.value.ticketProductList || []) &&
    TicketProcedure.equalList(
      ticketProcedureList.value,
      ticketRef.value.ticketProcedureList || []
    ) &&
    TicketRadiology.equalList(ticketRadiologyList.value, ticketRef.value.ticketRadiologyList || [])
  ) {
    return true
  }
...
const disableSendProduct = computed(() => {
  if (ticketRef.value.ticketStatus !== TicketStatus.Executing) {
    return true
  }
  if (ticketRef.value.deliveryStatus !== DeliveryStatus.Pending) {
    return true
  }
  if (!TicketProduct.equalList(ticketProductList.value, ticketRef.value.ticketProductList || [])) {
    return true
  }
  return false
})
  ...
  try {
    await TicketClinicApi.changeItemsMoney({
      ...
      ticketProductList: ticketProductList.value.filter((item, index) => {
        return !TicketProduct.equal(item, ticketRef.value.ticketProductList![index])
      }),
      ticketProcedureList: ticketProcedureList.value.filter((item, index) => {
        return !TicketProcedure.equal(item, ticketRef.value.ticketProcedureList![index])
      }),
      ticketRadiologyList: ticketRadiologyList.value.filter((item, index) => {
        return !TicketRadiology.equal(item, ticketRef.value.ticketRadiologyList![index])
      }),
    })
  } catch (e) {
...
const startSendProduct = async () => {
  ...
  try {
    ...
    await TicketClinicApi.sendProduct({ ticketId: ticketRef.value.id })
  } catch (error) {
...
const startReopenVisit = async () => {
  await TicketClinicApi.reopen(ticketRef.value.id)
}
...
const clickReopenVisit = () => {
  ModalStore.confirm({
    ...
    content: [
      ...(ticketRef.value.paid > 0
        ? [
            `- Số tiền nợ sẽ được hoàn trả, khi đóng hồ sơ lại sẽ ghi nợ trở lại`,
            `- Trừ nợ khách hàng: ${formatMoney(ticketRef.value.debt)}`,
          ]
...
const startPrint = async () => {
  try {
    ...
    const content = ticketSpaInvoiceHtmlContent(ticketRef.value)
...
<template>
  ...
  <div class="mt-4 flex gap-4">
    ...
    <VueButton
      v-if="
        [TicketStatus.Approved, TicketStatus.Executing].includes(ticketRef.ticketStatus) &&
        ticketRef.paid > ticketRef.totalMoney
      "
      icon="dollar"
      size="small"
      color="green"
