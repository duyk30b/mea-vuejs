...
const handleUpload = async () => {
  ...
  try {
    await FileProcedureApi.uploadExcelProcedureList(fileSelect.value)
    AlertStore.addSuccess('Cập nhật danh sách dịch vụ từ excel thành công !')
    emit('success')
    closeModal()
  } catch (error) {
    console.log('🚀 ~ ModalUploadProcedure.vue:43 ~ handleUpload ~ error:', error)
    removeFile()
  } finally {
...
const downloadExcelFileExample = async () => {
  try {
    await FileProcedureApi.downloadExcelFileExample()
  } catch (error) {
    console.log('🚀 ~ ModalUploadProcedure.vue:73 ~ downloadExcelFileExample ~ error:', error)
  }
}
    ...
    <div class="bg-white">
      <div class="pl-4 py-3 flex items-center" style="border-bottom: 1px solid #dedede">
        ...
        <div class="flex-1 font-medium" style="font-size: 16px; text-align: center">
          Upload danh sách dịch vụ từ Excel
        </div>
        ...
        <div class="mt-2 flex justify-between text-xs" style="color: #666; font-style: italic">
          <div class="flex gap-2">
            ...
            <div>
              ...
              <div>
                2. Mã dịch vụ đã có trên hệ thống sẽ bị ghi đè bởi thông tin trong excel: bao gồm
                tên dịch vụ, nhóm, giá ...
              </div>
              <div>3. Mã dịch vụ chưa có trên hệ thống sẽ được tạo mới</div>
              <div>4. Nếu có thông tin về nhóm dịch vụ, cần phải tạo nhóm dịch vụ trên hệ thống trước</div>
            </div>
          </div>
        </div>
      </div>
