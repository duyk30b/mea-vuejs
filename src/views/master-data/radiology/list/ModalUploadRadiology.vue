...
const handleUpload = async () => {
  ...
  try {
    await FileRadiologyApi.uploadExcelRadiologyList(fileSelect.value)
    AlertStore.addSuccess('Cập nhật danh sách phiếu CĐHA từ excel thành công !')
    emit('success')
    closeModal()
  } catch (error) {
    console.log('🚀 ~ ModalUploadRadiology.vue:43 ~ handleUpload ~ error:', error)
    removeFile()
  } finally {
...
const downloadExcelFileExample = async () => {
  try {
    await FileRadiologyApi.downloadExcelFileExample()
  } catch (error) {
    console.log('🚀 ~ ModalUploadRadiology.vue:73 ~ downloadExcelFileExample ~ error:', error)
  }
}
    ...
    <div class="bg-white">
      <div class="pl-4 py-3 flex items-center" style="border-bottom: 1px solid #dedede">
        ...
        <div class="flex-1 font-medium" style="font-size: 16px; text-align: center">
          Upload danh sách phiếu CĐHA từ Excel
        </div>
        ...
        <div class="mt-2 flex justify-between text-xs" style="color: #666; font-style: italic">
          <div class="flex gap-2">
            ...
            <div>
              <div>
                1.
                <strong>Mã phiếu CĐHA</strong>
                là duy nhất trên hệ thống và không thể trùng lặp
              </div>
              <div>
                2. Mã phiếu CĐHA đã có trên hệ thống sẽ bị ghi đè bởi thông tin trong excel: bao gồm
                tên phiếu CĐHA, nhóm, giá ...
              </div>
              <div>3. Mã phiếu CĐHA chưa có trên hệ thống sẽ được tạo mới</div>
              <div>
                4. Nếu có thông tin về nhóm phiếu CĐHA, cần phải tạo nhóm phiếu CĐHA trên hệ thống trước
              </div>
            </div>
          </div>
        </div>
