...
const selectProduct = async (productData?: Product) => {
  if (productData) {
    ...
    if (productData.warehouseIds === JSON.stringify([])) {
      return ModalStore.alert({
        title: 'Không thể nhập hàng cho sản phẩm này',
        content: [
          'Sản phẩm này không theo dõi số lượng tồn kho',
          `Cần sửa thông tin sản phẩm và chọn kho quản lý cho sản phẩm này`,
        ],
      })
    } else if (productData.warehouseIds === JSON.stringify([0])) {
    ...
    <div class="flex flex-wrap gap-4">
      <div style="flex-basis: 80%; flex-grow: 1">
        <div class="flex gap-1 flex-wrap">
          ...
          <span
            v-if="product.id && product.warehouseIds !== '[]'"
