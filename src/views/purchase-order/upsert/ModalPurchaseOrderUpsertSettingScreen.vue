...
const settingDisplay = ref<typeof store.SCREEN_PURCHASE_ORDER_UPSERT>(
  JSON.parse(JSON.stringify(store.SCREEN_PURCHASE_ORDER_UPSERT)),
)
const showModal = ref(false)
const saveLoading = ref(false)

const activeTab = ref(TABS_KEY.PURCHASE_ORDER_ITEMS)
...
const openModal = async () => {
  ...
  settingDisplay.value = JSON.parse(JSON.stringify(store.SCREEN_PURCHASE_ORDER_UPSERT))
}
...
const handleSave = async () => {
  ...
  try {
    ...
    await OrganizationService.saveSettings(SettingKey.SCREEN_PURCHASE_ORDER_UPSERT, settingData)
    AlertStore.addSuccess('Cập nhật cài đặt thành công', 1000)
    store.SCREEN_PURCHASE_ORDER_UPSERT = JSON.parse(settingData)
...
const handleUpdateTabShow = async (value: any) => {
  if (value === TABS_KEY.PURCHASE_ORDER_PAYMENT) {
...
<template>
  <VueModal v-model:show="showModal">
    <div class="bg-white">
      ...
      <div class="px-6 mt-4 purchaseOrder-upsert-setting-screen-tabs">
        <VueTabs v-model:tabShow="activeTab" @update:tabShow="handleUpdateTabShow">
          <template #menu>
            <VueTabMenu :tabKey="TABS_KEY.PURCHASE_ORDER_ITEMS">Cài đặt danh sách</VueTabMenu>
            <VueTabMenu :tabKey="TABS_KEY.PURCHASE_ORDER_PAYMENT">Cài đặt phiếu</VueTabMenu>
          </template>
          <template #panel>
            <VueTabPanel :tabKey="TABS_KEY.PURCHASE_ORDER_ITEMS">
                ...
                <table>
                  ...
                  <tbody>
                    <tr>
                      <td>
                        <InputCheckbox
                          v-model:checked="settingDisplay.purchaseOrderItemsTable.detail"
                        >
                    ...
                    <tr>
                      <td>
                        <InputCheckbox
                          v-model:checked="settingDisplay.purchaseOrderItemsTable.substance"
                        >
                          Hiển thị hoạt chất
                        </InputCheckbox>
                      </td>
                    ...
                    <tr>
                      <td>
                        <InputCheckbox
                          v-model:checked="
                            settingDisplay.purchaseOrderItemsTable.lotNumberAndExpiryDate
                          "
                        >
                          Hiển thị số lô và HSD
                        </InputCheckbox>
            ...
            <VueTabPanel :tabKey="TABS_KEY.PURCHASE_ORDER_PAYMENT">
              <div class="mt-4 table-wrapper">
                <table class="">
                  <thead>
                    <tr>
