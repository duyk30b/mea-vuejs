...
const startRegisterTicketClinic = (appointmentSelect: Appointment) => {
  ModalStore.confirm({
    title: 'Khách hàng đã đến khám ?',
    content: 'Hoàn thành lịch hẹn, bắt đầu đăng ký vào "Danh Sách Khám"',
    onOk: async () => {
      try {
        await AppointmentApi.registerTicketClinic(appointmentSelect.id)
        await startFetchData()
      } catch (error) {
        console.log('🚀 ~ file: AppointmentList.vue:152 ~ onOk: ~ error:', error)
      }
    },
  })
}
          ...
          <tr v-for="(appointment, index) in appointmentList" :key="index">
            ...
            <td class="text-center" style="width: 200px">
              <div
                ...
                <VueButton size="small" @click="() => startRegisterTicketClinic(appointment)">
                  Đăng ký khám
                </VueButton>
              </div>
