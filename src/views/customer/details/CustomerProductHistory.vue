      ...
      <table class="table">
        ...
        <tbody>
          ...
          <tr v-for="(ii, index) in invoiceItems" :key="index">
            ...
            <td>{{ ii.productBatch.product?.brandName }}</td>
            <td class="text-center">{{ ii.productBatch.batch }}</td>
            <td class="text-center">{{ timeToText(ii.productBatch.expiryDate) }}</td>
            <td class="text-center">{{ ii.productBatch.product?.unit.find(i => i.rate === 1)?.name }}</td>
            <td>{{ ii.quantity }}</td>
            <td>{{ formatNumber(ii.actualPrice) }}</td>
            <td>{{ formatNumber(ii.actualPrice * ii.quantity) }}</td>
          </tr>
        </tbody>
      </table>
