...
import {
  StatisticRadiologyApi,
  type StatisticTicketRadiologyResponseType,
} from '@/modules/statistics/statistic-radiology.api'
...
const statisticPagination = ref<StatisticTicketRadiologyResponseType[]>([])
const statisticTotal = ref<
  Pick<StatisticTicketRadiologyResponseType, 'count' | 'sumActualAmount' | 'sumCostAmount'>
>({
  count: 0,
  sumActualAmount: 0,
  sumCostAmount: 0,
})
const sortKey = ref<keyof StatisticTicketRadiologyResponseType>('count')
...
const startFetchData = async () => {
  try {
    ...
    const fetchResponse = await StatisticRadiologyApi.statisticTicketRadiology({
      filter: {
        createdAt:
    ...
    barData.labels.splice(
      ...
      ...statisticPagination.value.map((i) => i.radiology?.name),
    )
    ...
    console.log('🚀 ~ StatisticTicketRadiology.vue:112 ~ startFetchData ~ error:', error)
  } finally {
    loaded.value = true
  }
}
