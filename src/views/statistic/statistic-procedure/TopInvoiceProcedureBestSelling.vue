...
const startFetchHighQuantity = async () => {
  const data = await StatisticService.topInvoiceProcedureBestSelling({
...
const startFetchHighActualMoney = async () => {
  ...
  barData.labels.splice(0, data.length, ...data.map((i) => `${i.procedure.name} (${i.sumQuantity})`))
  ...
  <div class="flex flex-col" style="height: 100%">
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex justify-between items-center gap-2">
        <span style="font-size: 18px; font-weight: 500">Dịch vụ trong đơn: </span>
