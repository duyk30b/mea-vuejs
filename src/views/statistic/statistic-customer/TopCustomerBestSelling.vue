...
const typeBestSelling = ref<'topCountInvoice' | 'topRevenue' | 'topProfit'>('topRevenue')

const startFetchHighCountInvoice = async () => {
  const data = await StatisticService.topCustomerBestInvoice({
    fromTime: timeRanger.value?.[0].startOf('day').toISOString(),
    toTime: timeRanger.value?.[1].endOf('day').toISOString(),
    limit: isMobile ? 10 : 20,
    orderBy: 'countInvoice',
  })
  ...
  barData.datasets[0].data.splice(0, data.length, ...data.map((i) => i.countInvoice))
}

const startFetchHighRevenueInvoice = async () => {
  const data = await StatisticService.topCustomerBestInvoice({
    fromTime: timeRanger.value?.[0].startOf('day').toISOString(),
    toTime: timeRanger.value?.[1].endOf('day').toISOString(),
    limit: isMobile ? 10 : 20,
...
const startFetchData = async () => {
  try {
    ...
    } else if (typeBestSelling.value === 'topRevenue') {
      await startFetchHighRevenueInvoice()
        ...
        <div style="width: 120px">
          <VueSelect
            ...
            :options="[
              ...
              { text: 'Tiền đơn', value: 'topRevenue' },
