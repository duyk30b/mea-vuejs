...
watch(
  () => ticketClinic.value.ticketDiagnosis!.reason,
  (newValue, oldValue) => (ticketDiagnosis.value.reason = newValue || ''),
  { immediate: true }
)
watch(
  () => ticketClinic.value.ticketDiagnosis!.healthHistory,
  (newValue, oldValue) => (ticketDiagnosis.value.healthHistory = newValue || ''),
  { immediate: true }
)
watch(
  () => ticketClinic.value.ticketDiagnosis!.summary,
  (newValue, oldValue) => (ticketDiagnosis.value.summary = newValue || ''),
  { immediate: true }
)
watch(
  () => ticketClinic.value.ticketDiagnosis!.diagnosis,
  (newValue, oldValue) => (ticketDiagnosis.value.diagnosis = newValue || ''),
  { immediate: true }
)
watch(
  () => ticketClinic.value.ticketDiagnosis!.vitalSigns,
  (newValue, oldValue) => (vitalSigns.value = JSON.parse(newValue || JSON.stringify({}))),
  { immediate: true }
)
watch(
  () => ticketClinic.value.ticketDiagnosis!.imageIds,
  (newValue, oldValue) => (hasChangeImage.value = false),
  { immediate: true }
)

const hasChangeData = computed(() => {
  if (ticketClinic.value.ticketDiagnosis!.reason !== ticketDiagnosis.value.reason) {
    return true
  }
  if (ticketClinic.value.ticketDiagnosis!.healthHistory !== ticketDiagnosis.value.healthHistory) {
    return true
  }
  if (ticketClinic.value.ticketDiagnosis!.summary !== ticketDiagnosis.value.summary) {
    return true
  }
  if (ticketClinic.value.ticketDiagnosis!.vitalSigns !== JSON.stringify(vitalSigns.value)) {
    return true
  }
  if (ticketClinic.value.ticketDiagnosis!.diagnosis !== ticketDiagnosis.value.diagnosis) {
    return true
  }
  ...
  <div class="mt-4">
    ...
    <div class="mt-4">
      ...
      <ImageUpload
        ...
        :height="100"
        :rootImageList="
          (ticketClinic.ticketDiagnosis?.imageList || [])
