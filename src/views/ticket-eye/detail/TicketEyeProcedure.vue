...
const disabledButton = computed(() => {
  return (
    TicketProcedure.equalList(
      ...
      ticketEyeRef.value.ticketProcedureList || []
    ) || [TicketStatus.Debt, TicketStatus.Completed].includes(ticketEyeRef.value.ticketStatus)
  )
})

onMounted(async () => {
  console.log('🚀 ~ file: TicketEyeProcedure.vue:45 ~ onMounted')
  try {
...
const selectProcedure = (instance?: Procedure) => {
  if (instance) {
    ...
    ticketProcedure.ticketId = ticketEyeRef.value.id
...
const saveTicketProcedureList = async () => {
  await TicketEyeApi.updateTicketProcedureList({
    ticketId: ticketEyeRef.value.id,
    customerId: ticketEyeRef.value.customerId || 0,
...
<template>
  ...
  <div class="mt-4 flex justify-between">
    <div></div>
    <VueButton
      v-if="permissionIdMap[PermissionId.TICKET_EYE_UPDATE_TICKET_PROCEDURE_LIST]"
      color="blue"
