...
const openModal = async (view: PaymentViewType) => {
  ...
  ticketClone.value = Ticket.from(ticketEyeRef.value)
...
const startPrepayment = async () => {
  ...
  try {
    await TicketEyeApi.prepayment(ticketClone.value.id, money.value)
    emit('success')
    showModal.value = false
  } catch (error) {
...
const startRefundOverpaid = async () => {
  ...
  paymentLoading.value = true
  try {
    await TicketEyeApi.refundOverpaid(ticketClone.value.id, money.value)
    emit('success')
    showModal.value = false
  } catch (error) {
...
const startPayDebt = async () => {
  paymentLoading.value = true
  try {
    await TicketEyeApi.payDebt(ticketClone.value.id, money.value)
    emit('success')
    showModal.value = false
  } catch (error) {
