...
import ModalTicketEyeReturnProduct from './modal/ModalTicketEyeReturnProduct.vue'
import { ticketEyePrintInvoice } from './print-content/ticket-eye-print-invoice'

const modalTicketEyePayment = ref<InstanceType<typeof ModalTicketEyePayment>>()
const modalTicketEyeReturnProduct = ref<InstanceType<typeof ModalTicketEyeReturnProduct>>()
...
onMounted(async () => {
  console.log('🚀 ~ file: TicketEyeSummary.vue:46 ~ onMounted')
})

watch(
  () => ticketEyeRef.value.ticketProductConsumableList,
  (newValue, oldValue) => {
...
watch(
  () => ticketEyeRef.value.ticketProcedureList,
  (newValue, oldValue) => {
...
watch(
  () => ticketEyeRef.value.ticketRadiologyList,
  (newValue, oldValue) => {
...
const disableSendProduct = computed(() => {
  if (ticketEyeRef.value.ticketStatus !== TicketStatus.Executing) {
    return true
  }
...
const saveTicketItemsMoney = async () => {
  ...
  try {
    const prescriptionChangeList = ticketProductPrescriptionList.value.filter((item, index) => {
      return !TicketProduct.equal(item, ticketEyeRef.value.ticketProductPrescriptionList![index])
    })
    const consumableChangeList = ticketProductConsumableList.value.filter((item, index) => {
      return !TicketProduct.equal(item, ticketEyeRef.value.ticketProductConsumableList![index])
    })

    await TicketEyeApi.updateItemsMoney({
      ...
      ticketProcedureList: ticketProcedureList.value.filter((item, index) => {
        return !TicketProcedure.equal(item, ticketEyeRef.value.ticketProcedureList![index])
      }),
      ticketRadiologyList: ticketRadiologyList.value.filter((item, index) => {
        return !TicketRadiology.equal(item, ticketEyeRef.value.ticketRadiologyList![index])
      }),
    })
  } catch (e) {
    console.log('🚀 ~ file: TicketEyeSummary.vue:321 ~ saveTicketItemsMoney ~ e:', e)
  } finally {
...
const startSendProduct = async () => {
  ...
  try {
    ...
    await TicketEyeApi.sendProduct({ ticketId: ticketEyeRef.value.id })
  } catch (error) {
...
const startReopenVisit = async () => {
  await TicketEyeApi.reopen(ticketEyeRef.value.id)
}
...
const clickReopenVisit = () => {
  ModalStore.confirm({
    ...
    content: [
      ...(ticketEyeRef.value.debt > 0
        ? [
            `- Số tiền nợ sẽ được hoàn trả, khi đóng hồ sơ lại sẽ ghi nợ trở lại`,
            `- Trừ nợ khách hàng: ${formatMoney(ticketEyeRef.value.debt)}`,
          ]
...
const handleMenuActionClick = (menu: { key: string }) => {
  if (menu.key === 'RETURN_PRODUCT_LIST') {
    if ([TicketStatus.Debt, TicketStatus.Completed].includes(ticketEyeRef.value.ticketStatus)) {
      ...
      modalTicketEyeReturnProduct.value?.openModal()
    }
  }
...
const startPrint = async () => {
  try {
    ...
    const content = ticketEyePrintInvoice(ticketEyeRef.value)
    ...
    console.log('🚀 ~ file: TicketEyeSummary.vue:434 ~ startPrint ~ error:', error)
  }
}
</script>
<template>
  <ModalTicketEyePayment ref="modalTicketEyePayment" />
  <ModalTicketEyeReturnProduct ref="modalTicketEyeReturnProduct" />
  ...
  <div class="mt-4 flex gap-4">
    ...
    <VueButton
      v-if="
        [TicketStatus.Approved, TicketStatus.Executing].includes(ticketEyeRef.ticketStatus) &&
        ticketEyeRef.paid > ticketEyeRef.totalMoney
      "
      icon="dollar"
      size="small"
      color="green"
      @click="modalTicketEyePayment?.openModal(PaymentViewType.RefundOverpaid)">
      ...
      <a-dropdown>
        <template #overlay>
          <a-menu @click="handleMenuActionClick">
            ...
            <a-menu-item
              v-if="[TicketStatus.Debt, TicketStatus.Completed].includes(ticketEyeRef.ticketStatus)"
              key="REOPEN_VISIT">
              <span class="text-red-500">
                <FileSyncOutlined class="mr-2" />
                Mở lại phiếu khám
  ...
  <div class="mt-8 flex gap-4">
    ...
    <VueButton
      v-if="permissionIdMap[PermissionId.TICKET_EYE_UPDATE_ITEMS_MONEY]"
      class="ml-auto"
      color="blue"
