...
import { TicketEyeApi, ticketEyeList } from '../../../modules/ticket-eye'
import { DTimer } from '../../../utils'
import ModalCustomerDetail from '../../customer/detail/ModalCustomerDetail.vue'
import TicketEyeStatusTag from '../TicketEyeStatusTag.vue'
import ModalTicketEyeRegister from './ModalTicketEyeRegister.vue'
...
const page = ref(1)
const limit = ref(Number(localStorage.getItem('TICKET_EYE_PAGINATION_LIMIT')) || 10)
const total = ref(0)
...
const startFetchData = async () => {
  try {
    ...
    ticketEyeList.value = data
    total.value = meta.total
  } catch (error) {
    console.log('🚀 ~ file: TicketEyeList.vue:84 ~ startFetchData ~ error:', error)
  } finally {
...
onMounted(async () => {
  try {
    ...
    console.log('🚀 ~ file: TicketEyeList.vue:98 ~ onMounted ~ error:', error)
...
const changePagination = async (options: { page?: number; limit?: number }) => {
  ...
  if (options.limit) {
    ...
    localStorage.setItem('TICKET_EYE_PAGINATION_LIMIT', String(options.limit))
  }
  await startFetchData()
}
...
const handleModalTicketEyeRegisterSuccess = (data: { ticket: Ticket }) => {
  const ticketFind = ticketEyeList.value.find((i) => i.id === data.ticket.id)
  if (!ticketFind) {
    ticketEyeList.value.unshift(data.ticket)
  }
}
...
const handleClickDestroyDraft = async (ticketId: number) => {
  ModalStore.confirm({
    ...
    onOk: async () => {
      try {
        await TicketEyeApi.destroyDraftSchedule(ticketId)
        await startFetchData()
        AlertStore.addSuccess('Xóa phiếu khám thành công')
      } catch (error) {
        console.log('🚀 ~ file: TicketEyeList.vue:179 ~ onOk: ~ error:', error)
      }
    },
  })
}
...
<template>
  <ModalTicketEyeRegister
    ref="modalTicketEyeRegister"
    @success="handleModalTicketEyeRegisterSuccess" />
  ...
  <div class="page-main">
    ...
    <div class="page-main-table table-wrapper">
      <table>
        ...
        <tbody v-else>
          <tr v-if="ticketEyeList.length === 0">
            <td colspan="20" class="text-center">No data</td>
          </tr>
          <tr v-for="(ticket, index) in ticketEyeList" :key="index">
            <td class="text-center">
              <div class="flex gap-4 justify-center">
                <router-link :to="{ name: 'TicketEyeDetailContainer', params: { id: ticket.id } }">
