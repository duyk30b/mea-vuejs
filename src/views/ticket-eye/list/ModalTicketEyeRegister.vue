...
import { TicketEyeApi } from '../../../modules/ticket-eye'
...
const ticketEyeCreateForm = ref<InstanceType<typeof HTMLFormElement>>()
...
const selectCustomer = async (customerSelect: Customer) => {
  if (customerSelect) {
    try {
      ...
      console.log('🚀 ~ file: ModalTicketEyeRegister.vue:81 ~ selectCustomer ~ error:', error)
    }
  } else {
    customer.value = Customer.blank()
  }
...
const handleRegisterVisit = async () => {
  ...
  try {
    const ticketResponse = await TicketEyeApi.register({
    ...
    console.log('🚀 ~ file: ModalTicketEyeRegister.vue:144 ~ handleRegisterVisit ~ error:', error)
  } finally {
...
<template>
  <VueModal v-model:show="showModal">
    <form ref="ticketEyeCreateForm" class="bg-white" @submit.prevent="handleRegisterVisit">
