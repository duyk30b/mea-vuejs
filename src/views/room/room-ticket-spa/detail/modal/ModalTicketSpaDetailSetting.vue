...
const settingDisplay = ref<typeof settingStore.TICKET_SPA_DETAIL>(
  JSON.parse(JSON.stringify(settingStore.TICKET_SPA_DETAIL)),
)
...
const activeTab = ref(TABS_KEY.PRODUCT)

const openModal = async () => {
  showModal.value = true
  settingDisplay.value = JSON.parse(JSON.stringify(settingStore.TICKET_SPA_DETAIL))
...
pickupStrategyOptions.forEach((i) => {
  if (i.value === MeService.settingMapRoot.value.TICKET_SPA_DETAIL.product.pickupStrategy) {
...
const clearWarehouseIdList = () => {
  if (settingDisplay.value.product.warehouseIdList.length === 0) {
    settingDisplay.value.product.warehouseIdList = [0]
  }
  if (settingDisplay.value.product.warehouseIdList.includes(0)) {
    settingDisplay.value.product.warehouseIdList = [0]
  }
}
...
const handleSave = async () => {
  ...
  try {
    ...
    await OrganizationService.saveSettings(SettingKey.TICKET_SPA_DETAIL, settingData)
    AlertStore.addSuccess('Cập nhật cài đặt thành công', 1000)
    settingStore.TICKET_SPA_DETAIL = JSON.parse(settingData)
    emit('success')
    showModal.value = false
  } catch (error) {
    ...
    <div class="bg-white">
      ...
      <div class="px-4 mt-4 invoice-upsert-setting-screen-tabs">
        <VueTabs :tabShow="activeTab">
          <template #menu>
            <VueTabMenu :tabKey="TABS_KEY.PRODUCT">Sản phẩm</VueTabMenu>
          </template>
          <template #panel>
            <VueTabPanel :tabKey="TABS_KEY.PRODUCT">
              <div class="mt-4 pb-20 table-wrapper">
                <table class="">
                  ...
                  <tbody>
                    <tr>
                      <td style="width: 30%">Kho tìm kiếm sản phẩm</td>
                      <td>
                        <div>
                          <a-select
                            v-model:value="settingDisplay.product.warehouseIdList"
                            mode="multiple"
                            style="width: 100%"
                    ...
                    <tr>
                      <td colspan="2">
                        <InputCheckbox
                          v-model:value="settingDisplay.product.searchIncludeZeroQuantity"
                          type-parser="number"
                    ...
                    <tr>
                      ...
                      <td>
                        <div>
                          <InputSelect
                            v-model:value="settingDisplay.product.pickupStrategy"
