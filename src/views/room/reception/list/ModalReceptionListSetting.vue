...
const settingDisplay = ref<typeof store.RECEPTION_TICKET_LIST>(
  JSON.parse(JSON.stringify(store.RECEPTION_TICKET_LIST)),
)
...
const openModal = async () => {
  ...
  settingDisplay.value = JSON.parse(JSON.stringify(store.RECEPTION_TICKET_LIST))
...
const handleSave = async () => {
  ...
  try {
    ...
    await OrganizationService.saveSettings(SettingKey.RECEPTION_TICKET_LIST, settingData)
    AlertStore.addSuccess('Cập nhật cài đặt thành công', 500)
    store.RECEPTION_TICKET_LIST = JSON.parse(settingData)
