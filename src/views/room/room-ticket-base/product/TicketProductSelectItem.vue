...
const pickupStrategy = ref(MeService.getPickupStrategySpa().product)
      ...
      filter: {
        ...
        $AND: [
          ...
          },
          {
            $OR: [
              {
                quantity: settingStore.TICKET_SPA_DETAIL.product.searchIncludeZeroQuantity
                  ? undefined
                  : { NOT: 0 },
              },
              { warehouseIds: '[]' },
        ...
        warehouseIds: (productWarehouseIds) => {
          try {
            const warehouseIdAcceptList = settingStore.TICKET_SPA_DETAIL.product.warehouseIdList
...
const selectProduct = async (productSelect?: Product) => {
  if (productSelect) {
    const priorityList = (ticketRoomRef.value.ticketProductList || []).map((i) => {
      return i.priority
    })
    ...
    temp.type = TicketProductType.Other
    ...
    temp.warehouseIds = JSON.stringify(settingStore.TICKET_SPA_DETAIL.product.warehouseIdList) // set tạm trước thôi, tí nữa tính toán lại
    ...
    if (productSelect.productType === ProductType.SplitBatch) {
      const warehouseIdAcceptList: number[] = settingStore.TICKET_SPA_DETAIL.product.warehouseIdList
...
const addTicketProduct = async () => {
  ...
  if (product?.warehouseIds !== '[]' && pickupStrategy.value !== PickupStrategy.NoImpact) {
    ...
    } else if (ticketProduct.value.batchId && ticketProduct.value.quantity > batch!.quantity) {
      AlertStore.addWarning(
  ...
  // tính lại costAmount
  ticketProduct.value.costAmount = ticketProduct.value.quantity * (product?.costPrice || 0)
  emit('success', [ticketProduct.value])
  clear()
  ...
  <form class="mt-4 flex flex-wrap gap-4" @submit.prevent="(e) => addTicketProduct()">
      ...
      <div class="flex gap-1 flex-wrap">
        ...
        <div v-if="ticketProduct.productId">
          ...
          >
            Tồn:
            <b>
              {{ ticketProduct.product?.unitQuantity }}
        ...
        <a
          v-if="userPermission[PermissionId.PRODUCT_UPDATE] && ticketProduct.product?.id"
          @click="modalProductUpsert?.openModal(ticketProduct.product.id)"
        >
    ...
    <div style="flex-grow: 1; flex-basis: 300px">
      ...
      <div class="flex">
        ...
        <div class="flex-1">
          <InputNumber v-model:value="ticketProduct.unitQuantity" :validate="{ gt: 0 }" required />
        </div>
      </div>
    </div>
    ...
    <div style="flex-grow: 1; flex-basis: 300px">
      ...
      <div class="flex">
        <div class="flex-1">
          <InputNumber
            v-model:value="ticketProduct.unitActualPrice"
            :validate="{ gte: 0 }"
            required
