...
const clickDestroy = async () => {
  if (ticketProductOrigin.deliveryStatus === DeliveryStatus.Delivered) {
    return ModalStore.alert({
      ...
      content: [
        ...
        '- Chỉ có thể hoàn trả sản phẩm nếu bắt buộc phải thay đổi số lượng ?',
      ],
    })
  }
  ...
  if ([TicketStatus.Debt, TicketStatus.Completed].includes(ticketRoomRef.value.status)) {
    return ModalStore.alert({
      title: 'Không thể xóa sản phẩm ?',
      content: [
        '- Phiếu khám đã đóng không thể xóa sản phẩm',
  ...
  ModalStore.confirm({
    ...
    content: [
      '- Hệ thống sẽ xóa sản phẩm này khỏi phiếu khám',
      '- Dữ liệu đã xóa không thể phục hồi, bạn vẫn muốn xóa ?',
    ],
    onOk: async () => {
      try {
        await TicketChangeProductApi.destroyTicketProduct({
          ticketId: ticketRoomRef.value.id,
          ticketProductId: ticketProductOrigin.id,
        })
        closeModal()
      } catch (error) {
        console.log('🚀 ~ ModalTicketProductUpdate.vue:219 ~ clickDestroy ~ error:', error)
      }
    },
  })
}
...
const updateTicketProduct = async () => {
  ...
  try {
    ...
    await TicketChangeProductApi.updateTicketProduct({
      ticketId: ticketRoomRef.value.id,
