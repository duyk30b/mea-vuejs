...
onMounted(async () => {
  ...
  Promise.all([
    productStore.fetchAll({ relation: { productBatches: true } }),
...
const selectProduct = async (data?: Product) => {
  console.log('🚀 ~ file: InvoiceItemCreate.vue:89 ~ selectProduct ~ data:', data)
  if (data) {
...
const selectProcedure = (data?: Procedure) => {
  const snapProcedure = Procedure.fromInstance(data || Procedure.blank())
  const ii = new InvoiceItem()
        ...
        <div style="height: 40px">
          ...
          >
            <template #each="{ item: { brandName, substance, unit, quantity } }">
              <div>
                <b>{{ brandName }}</b> - <b>{{ quantity }}</b> {{ unit?.[0].name }}
              </div>
              <div>{{ substance }}</div>
            </template>
          </InputOptions>
      ...
      <div class="mt-4 flex flex-wrap gap-4">
        <div style="flex: 1; flex-basis: 400px">
          <div>
            Lô hàng
            <span
              v-if="invoiceItem.productBatch?.expiryDate && invoiceItem.productBatch?.expiryDate < Date.now()"
          ...
          <a-select
            v-model:value="invoiceItem.referenceId"
            class="w-full"
            :options="product.productBatches.map((i: ProductBatch) => ({
              value: i.id,
              label:
                `${i.batch} - ${timeToText(i.expiryDate, 'DD/MM/YYYY')}` +
                ` - SL: ${i.quantity} ${product.unit?.[0].name} - Giá ${formatMoney(i.retailPrice)}`,
            }))
            "
    ...
    >
      <div>
        ...
        <InputOptions
          ...
          v-model:searchText="procedure.name"
    ...
    <div class="flex flex-wrap gap-4">
      ...
      >
        ...
        <div class="flex">
          ...
          >
            <a-select-option value="retailPrice">
              Giá bán lẻ
            </a-select-option>
            <a-select-option
            ...
            <a-select-option v-if="organizationStore.SCREEN_INVOICE_UPSERT.invoiceItemInput.costPrice" value="costPrice">
              Giá nhập
            </a-select-option>
          </a-select>
          <div class="flex-1">
      ...
      >
        <div>
          ...
          <template v-if="tabsKey === 'product'">
            <span v-if="invoiceItem.unit.rate !== 1">
              (Quy đổi: <b>{{ invoiceItem.quantity }}</b> {{ product.unit.find((i) => i.rate === 1)?.name }})
            </span>
          </template>
        </div>
        <div class="flex">
      ...
      >
        <div>
          ...
          <template v-if="tabsKey === 'product'">
            <span
              v-if="(invoiceItem.discountType === DiscountType.Percent && invoiceItem.discountPercent !== 0) ||
