...
export const invoiceHtmlContent = (invoice: Invoice) => {
	...
	const rowInvoiceItem = invoice.invoiceItems
		.map((item, index) => {
			...
			let expectedPrice = ''
			if (organizationStore.SCREEN_INVOICE_DETAIL.invoiceItemsTable.expectedPrice && item.discountMoney) {
	...
	let totalItemMoney = ''
	if (organizationStore.SCREEN_INVOICE_DETAIL.paymentInfo.totalItemMoney) {
		totalItemMoney = `<tr>
			<td colspan="5" style="text-align: right"><b>Tiền hàng</b></td>
			<td style="text-align: right"><b>${formatMoney(invoice.totalItemMoney)}</b></td>
		</tr>`
	}
				...
				<table class="print-invoice-item-list">
					...
					<tbody>
						...
						<tr>
							...
							<td style="text-align: right"><b>${formatMoney(invoice.totalMoney)}</b></td>
						</tr>
					</tbody>
				</table>
