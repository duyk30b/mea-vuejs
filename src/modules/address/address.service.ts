  ...
  static search = ESFunction.debounceAsync(
    async (text: string, options: { limit: 20 }): Promise<Address[]> => {
      ...
      for (let i = 0; i < AddressService.addressAll.length; i++) {
        ...
        if (DString.customFilter(address.province, text, 1)) {
          result.push(address)
          continue
        }
        if (DString.customFilter(address.ward, text, 1)) {
          result.push(address)
          continue
        }
        if (DString.customFilter(`${address.ward} - ${address.province}`, text, 1)) {
          result.push(address)
          continue
        }
      }
