...
export const ticketClinicPrintPrescriptionTemplate = (
  ticket: Ticket,
  doctorName: string,
  htmlString: string
) => {
  ...
  // Tạo một hàm động để thay thế các biến trong template
  ...
  return compiledTemplate(ticket, organization, doctorName, DTimer, formatPhone)
}

export const ticketClinicPrintPrescription = (ticket: Ticket, doctorName: string) => {
  const meStore = useMeStore()
  const organization = meStore.organization
  return `
  <head>
